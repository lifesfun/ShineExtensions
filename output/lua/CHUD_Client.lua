...
Script.Load("lua/CHUD_LowHealth.lua")
...
function ApplyCHUD(script, scriptName)
		...
		if scriptName == "GUIMarineHUD" then
			...
			if CHUDSettings["minimap"] then
				script.minimapScript:ShowMap(true)
			else
				if Client.GetOptionInteger("hudmode", kHUDMode.Full) == kHUDMode.Full then
					script.minimapScript:ShowMap(false)
					script.minimapBackground:SetIsVisible(false)
				end
				script.locationText:SetIsVisible(false)
				script.commanderName:SetIsVisible(false)
				if CHUDSettings["showcomm"] then
...
function OnCommandCHUDHelp()
	...
	Shared.Message("chud_mingui: Removes backgrounds/scanlines from all UI elements (still not finished)")
...
function AnnounceCHUD()
	...
	GetCHUDSettings()
	// This needs to be done after the client has loaded
