        ...
        for p = 1, #self.pathPoints do
            ...
            if arrowDist >= 5 then
                ...
                if trace.fraction ~= 1 then
                    ...
                    if distToPlayer > kArrowMinDistToPlayerSquared and distToTarget > kArrowMinDistToTargetSquared and not (CHUDSettings["minwps"] or not CHUDSettings["wps"]) then // MOD
...
local function AnimateFinalWaypoint(self)
	// MOD START
	local iscomm = Client.GetLocalPlayer():isa("Commander")
	// MOD END
	
    local finalWaypointData = PlayerUI_GetFinalWaypointInScreenspace()
    local showWayPoint = not PlayerUI_GetIsConstructing() and not PlayerUI_GetIsRepairing() and (CHUDSettings["wps"] or iscomm) // MOD
    
    local fullHUD = Client.GetOptionInteger("hudmode", kHUDMode.Full) == kHUDMode.Full
    
    self.animatedCircle:SetIsVisible(showWayPoint and fullHUD and not CHUDSettings["minwps"]) // MOD
    ...
    if finalWaypointData then
        ...
        self.finalDistanceText:SetIsVisible(fullHUD and not CHUDSettings["minwps"]) // MOD
        self.finalNameText:SetIsVisible(fullHUD and not CHUDSettings["minwps"]) // MOD
		...
		// MOD START
		// Show the waypoint if it's an attack waypoint in the disabled WPs mode
		if not CHUDSettings["wps"] and type == kTechId.Attack then
			if not CHUDSettings["minwps"] then
				self.animatedCircle:SetIsVisible(fullHUD)
			end
			self.finalWaypoint:SetIsVisible(true)
		end
		// MOD END
        ...
        self.finalWaypoint:SetSize(minsize) // MOD
